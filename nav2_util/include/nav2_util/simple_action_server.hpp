...
public:
   ...
   * 
   * explicit 用于修饰构造函数，表明这个构造函数是显式的，禁止隐式类型转换。也就是说，在创建 SimpleActionServer 类的对象时，
   * 必须显式地调用这个构造函数，不能通过其他隐式的方式（比如将一个可以转换为 NodeT 类型的表达式自动转换后去调用构造函数等情况）来调用，
   * 这样可以避免一些意外的、不符合预期的类型转换行为，让代码的行为更加明确和可预测。
   * 
   * 这是 C++ 11 引入的委托构造（Delegating Constructor）的语法形式。
   * 在这里，当前构造函数（带有多个参数的那个构造函数）将构造的具体工作委托给了另一个构造函数（
   * 也就是代码中调用的这个同样名为 SimpleActionServer 的构造函数，它应该还有另外的定义，接受更多与节点接口相关的参数等）
   */
    ...
    spin_thread_(spin_thread)
  {
    ...
    /*
      rclcpp::CallbackGroupType 是一个枚举类型，定义了不同类型的回调组。
      MutuallyExclusive（互斥型）是其中一种类型，
      它的作用在于当多个回调函数被分配到同一个 MutuallyExclusive 类型的回调组中时，
      在执行器（Executor，后续会提到）执行回调时，同一时刻只会执行该回调组中的一个回调函数，
      其他回调函数需要等待当前执行的回调函数完成后才能被执行
    */
    }
    // https://docs.ros2.org/latest/api/rclcpp_action/create__server_8hpp.html
    ...
    /*
      rclcpp::executors::SingleThreadedExecutor 的作用是提供一个执行环境，
      它可以按照一定的顺序去轮询和执行分配到各个回调组中的回调函数。
      由于是单线程执行器，它在同一时刻只会执行一个回调函数，依次处理各个回调组中的回调任务，
      避免了多线程并发带来的复杂性和潜在的问题（如线程同步、死锁等），
      适合于对回调执行顺序要求较高且不需要复杂并行处理的场景，
      比如这个动作服务器相关的回调处理场景，确保回调函数按照一定顺序依次执行。
   ...
   * 
   * handle_goal 是处理客户端发送的 目标（goal）请求 的回调函数。
     它的作用是决定是否接受客户端发来的目标请求。
     返回值是 rclcpp_action::GoalResponse 类型，
     这个值告诉 Action Server 是否接受或拒绝该目标。

     返回值为 rclcpp_action::GoalResponse，可以是以下三种：
     GoalResponse::REJECT: 拒绝该目标。
     GoalResponse::ACCEPT_AND_EXECUTE: 接受目标并立即开始执行。
     GoalResponse::ACCEPT_AND_DEFER: 接受目标，但稍后再决定是否执行（延迟执行）。
   */
   ...
   * 
   * handle_cancel 是处理客户端发送的 取消（cancel）请求 的回调函数。
     它的作用是决定是否允许取消目标。
     返回值为 rclcpp_action::CancelResponse，可以是以下两种：
     CancelResponse::ACCEPT: 接受取消请求。
     CancelResponse::REJECT: 拒绝取消请求。
   */
   ...
   * 
   * 
   * handle_accepted 是处理客户端发送的目标被接受后
   * （ACCEPT_AND_EXECUTE 或 ACCEPT_AND_DEFER）的回调函数。
     它的作用是启动目标的执行逻辑，
     通常是将目标加入到异步执行线程或触发执行任务的入口。

  异步执行:
    handle_accepted 的逻辑通常需要异步执行，因为它负责目标的具体执行过程。
    直接在回调中同步执行可能会阻塞主线程，影响其他回调的处理。
    检查取消状态:
    在目标执行过程中，应该定期检查 goal_handle->is_canceling()，以响应取消请求。
    任务完成状态:
    在任务完成时，必须调用以下方法之一，通知 Action 客户端任务状态：
    goal_handle->succeed(result)：表示目标成功完成。
    goal_handle->abort(result)：表示目标因错误中止。
    goal_handle->canceled(result)：表示目标因取消请求被中止。

    当handle_goal函数接受了一个目标（返回rclcpp_action::GoalResponse::ACCEPT）后，
    动作服务器会触发handle_accepted函数。
    这个函数主要用于启动与执行目标相关的实际操作，例如在机器人控制场景中，
    它可能会启动一个新的线程或者触发一个执行器来开始执行机器人的动作任务。
   */
   ...
   */
  const std::shared_ptr<const typename ActionT::Goal> accept_pending_goal()   // 判断并终止 current_handle_ ， 然后将 pending_handle_ 赋值给 current_handle_
  {
    std::lock_guard<std::recursive_mutex> lock(update_mutex_);
