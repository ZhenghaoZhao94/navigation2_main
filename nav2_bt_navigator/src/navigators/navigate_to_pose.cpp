...
//  获取 并返回 行为树 xml 文件 navigate_to_pose_w_replanning_and_recovery.xml
std::string
  ...
  rclcpp_lifecycle::LifecycleNode::WeakPtr parent_node)
{
  ...
  if (!node->has_parameter("default_nav_to_pose_bt_xml")) {
    std::string pkg_share_dir =
      ament_index_cpp::get_package_share_directory("nav2_bt_navigator");  // ament_index_cpp::get_package_share_directory("nav2_bt_navigator")
                                                                          //  函数的主要作用是获取指定 ROS2 软件包（在这个例子中是 "nav2_bt_navigator"）的共享目录路径。
                                                                          //在 ROS2 项目中，每个软件包通常都有一个共享目录，这个目录用于存放一些与该软件包相关的共享资源，
                                                                          // 比如配置文件（.yaml、.xml 等格式）、行为树定义文件（如果涉及行为树相关功能的话）、模型文件、
                                                                          // 插件描述文件等各类在软件包运行过程中需要被多个组件访问或者需要对外提供的资源。
...
NavigateToPoseNavigator::goalReceived(ActionT::Goal::ConstSharedPtr goal)
{
  ...
  if (!bt_action_server_->loadBehaviorTree(bt_xml_filename)) {    // 根据目标点 带的 行为树 加载行为树
    RCLCPP_ERROR(
...
/*
  函数的主要作用是在导航过程中的每个循环周期内，收集并整理当前导航相关的各种状态信息，
  如机器人位姿、路径剩余距离、预估剩余时间、恢复次数以及已用导航时间等，
  然后将这些信息封装成反馈消息发布出去，使得整个导航系统中的其他相关组件能够实时了解导航的进展情况，
  以便进行有效的协调和控制，保障导航任务能够顺利朝着目标完成。
  同时通过合理的异常处理和错误日志记录机制，增强函数的稳定性和可维护性。
*/
void
NavigateToPoseNavigator::onLoop()   // 
{
  ...
  try {
    ...
    // Find the closest pose to current pose on global path 查找当前位姿在路径上的最近点索引
...
/*  订阅目标点并设置目标点
在接收到导航目标信息后，围绕目标位姿和当前机器人位姿进行一系列的初始化操作，
包括获取、转换坐标系、更新相关状态和在共享数据的黑板上记录目标位姿等，
同时通过返回值准确反馈初始化过程是否成功，为后续顺利开展导航任务奠定基础，
确保整个导航过程能够基于正确的初始条件有序进行。
*/
bool
NavigateToPoseNavigator::initializeGoalPose(ActionT::Goal::ConstSharedPtr goal)
{
  geometry_msgs::msg::PoseStamped current_pose;  // 获取当前位姿
  if (!nav2_util::getCurrentPose(
      current_pose, *feedback_utils_.tf,
      feedback_utils_.global_frame, feedback_utils_.robot_frame,
      feedback_utils_.transform_tolerance))
  ...
  geometry_msgs::msg::PoseStamped goal_pose;  // 获取 全局坐标系下的 目标点位姿
