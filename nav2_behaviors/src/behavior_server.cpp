...
bool
BehaviorServer::loadBehaviorPlugins()  // 以ros2 plugin 形式  创建 插件实例 放入 behaviors_
{
...
void BehaviorServer::setupResourcesForBehaviorPlugins()  // 根据模式需求 初始化设置 地图相关订阅器
{
...
BehaviorServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  std::vector<pluginlib::UniquePtr<nav2_core::Behavior>>::iterator iter;  // 激活所有插件
