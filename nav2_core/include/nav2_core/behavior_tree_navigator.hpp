...
public:
    ...
    std::shared_ptr<nav2_util::OdomSmoother> odom_smoother) final
  {
    ...
    std::string default_bt_xml_filename = getDefaultBTFilepath(parent_node);  // 行为树 xml 文件路径
   ...
   * @return bool If successful
   * 
   * 当一个虚函数被声明为 final 时，意味着在任何派生类中都不允许重写（重新定义）这个函数。
   * 这样做的目的主要是为了确保类层次结构中该函数的行为确定性，
   * 防止派生类意外地修改了这个函数原本期望的实现逻辑，从而破坏了类的整体设计和功能。
   */
...
protected:
  ...
  /**
   * @param Method to configure resources.   // 虚函数 不同子类 重写
   */
